<template>
  <div class="TUI-contact" :class="[!isPC && 'TUI-contact-H5']">
    <aside
      class="TUI-contact-left"
      v-if="isPC || (!isPC && !currentGroup.groupID && !currentFriend.userID)"
    >
    ...
    <main
      ...
      v-if="isPC && !currentGroup.groupID && !currentFriend.userID"
    >
      <slot />
    </main>
  </div>
...
const myFriendList = ref<Array<any>>([]);
...
const handleSearchGroup = async () => {
  ...
  try {
    ...
    const message = TUITranslateService.t("TUIGroup.该群组不存在");
    Toast({
      message,
      type: TOAST_TYPE.ERROR,
    });
